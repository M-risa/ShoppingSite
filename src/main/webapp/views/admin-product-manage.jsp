...
<script src="${pageContext.request.contextPath}/script/script.js"
	id="adminScript" data-context-path="${pageContext.request.contextPath}"
	defer></script>
...
<body id="admin-body">
	...
	<form id="productActionForm" method="post"
		...
		<div class="adminProductContainer">
			...
			%>
			<div class="productCard" id="product-<%=product.getProductId()%>">
				...
				<div class="productInfoArea">
					...
					<div class="productDatesArea">
						<div>
							登録日：<%=product.getRegistDate() != null ? product.getRegistDate().replaceAll("\\.\\d+$", "") : "-"%></div>
						<div>
							更新日：<%=product.getUpdateDate() != null ? product.getUpdateDate().replaceAll("\\.\\d+$", "") : "-"%></div>
					</div>
				</div>
			...
			<div style="text-align: center; padding: 40px 0; color: #888888;">
				登録されている商品がありません。「登録」ボタンから商品を追加してください。</div>
			<%
			}
			%>
